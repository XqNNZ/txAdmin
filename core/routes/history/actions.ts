...
async function handleRevokeAction(ctx: AuthedCtx): Promise<GenericApiOkResp> {
    ...
    if (ctx.admin.hasPermission('players.warn')) {
        perms.push('warn');
        perms.push('occurrence');
    }
    ...
    try {
        ...
        action = txCore.database.actions.revoke(actionId, ctx.admin.name, perms, revokeReason) as DatabaseActionType;
